...
resource ApplyConfigPackage 'Microsoft.Compute/virtualMachines/extensions@2021-04-01' = {
  parent: virtualMachine
  ...
  properties: {
    ...
    settings: {
      commandToExecute: 'powershell.exe -Command "Set-DnsClientServerAddress \'Ethernet\' -ServerAddresses (\'168.63.129.16\'); Clear-DnsClientCache; if (!(Test-Path -PathType Container -Path c:\\tools)) { New-Item -Path C:\\Tools -ItemType Directory }; Get-DnsClientServerAddress | Out-File c:\\Tools\\setup.log -encoding default; Invoke-WebRequest -Uri https://github.com/dloder0/SecPrac/raw/main/toolstest.zip -OutFile c:\\tools\\setup.zip; if (Test-Path -PathType Leaf -Path c:\\tools\\setup.zip) { Expand-Archive -Path C:\\Tools\\setup.zip -DestinationPath C:\\Tools -Force; if (Test-Path -PathType Leaf -Path c:\\tools\\invoke-setup.ps1) { & c:\\tools\\invoke-setup.ps1 }}"'
    }
  }
}
